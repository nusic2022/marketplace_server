			...
			if (!error && response.statusCode == 200) {
				...
				for(let i = 0; i < data.data.items.length; i++) {
					...
					try {
						...
						console.log('Error: Token Uri must be IPFS format, the uri length must be 65 to 96');
					}

					const setTransferableData = params.setTransferable.topic !== undefined ? await parseData(web3, item, params.setTransferable, gameParams.chainId, gameParams.gameName) : false;
					if(setTransferableData) parseData.push(setTransferableData);
				...
				await sleep(500);
				for(let i = 0; i < parsedData.length; i++) {
					if(parsedData[i].action === "setTransferable") {
